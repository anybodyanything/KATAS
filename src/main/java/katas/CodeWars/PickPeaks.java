...
public class PickPeaks {
    ...
    pairs:"pos"and"peaks". If there is no peak in the given array,
    ...
    public Map<String, List<Integer>> getPeaks(int[] arr) {
        Map<String, List<Integer>> pickedPeaks = new HashMap<>();
        List<Integer> pos = new ArrayList<>();
        List<Integer> peaks = new ArrayList<>();
        pickedPeaks.put("pos", pos);
        pickedPeaks.put("peaks", peaks);

        if (arr == null || arr.length < 3) return pickedPeaks;
        ...
        for (int i = 1; i < arr.length - 1; i++) {
            ...
            if (plateauStart != null && arr[i] > arr[i +1]){
